...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['resume']) && $_FILES['resume']['error'] == 0) {
        ...
        if (in_array($resume_type, $allowed_types) && $resume_size <= $max_file_size) {
            // Upload the resume to a folder (e.g., 'uploads/resumes')
            ...
            if (move_uploaded_file($resume_tmp_name, $resume_path)) {
                ...
                if (mysqli_query($conn, $query)) {
                    echo "<script>alert('Application submitted successfully!'); window.location.href='job-details.php?job_id=$job_id';</script>";
                    exit;
                } else {
